create or replace procedure nj_v2006_create_data_tables(year text)
language plpgsql
as
$body$
declare
...
begin
    execute format($a$create unlogged table if not exists %1$s.crash (
        ...
        route text,
        -- TODO: Ask about this.
        -- Values in files do not match codes in FFM2006 (p. 9).
        -- given codes: (A,B,C,M,P,S,T,U,L,W) 
        -- e.g. 2015 contains (1,2,3,A,B,M,S,T,Z)
        route_suffix text,
        sri text,
    ...
    execute format($e$create unlogged table if not exists %1$s.vehicle (
        ...
        towed text references %2$s.driven_left_towed (code),
